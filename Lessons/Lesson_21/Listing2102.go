func read_json_file(filepath string) {
   ...
   // split the content of the file based on lines (each line is a review)   scanner.Split(bufio.ScanLines)
   for scanner.Scan() {
      // We can iterate through and display each review
      //fmt.Println(scanner.Text()) // This is commented; otherwise, it will
                                    // print the entire file, which will take
                                    // a while. Uncomment if you want to see
